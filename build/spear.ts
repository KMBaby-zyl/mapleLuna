...
const config = {
  ...
  sixtyPercentScrollCost: 110_000,
};
...
function writeLogFile(
  ...
  const logDir = join(
    process?.cwd?.() ?? ".",
    "build",
    `spear-target-${targetTotalAttack}`,
  );
...
function runOne(baseAttack: number, weaponCost: number, targetAttackGain: number): string {
  ...
  log(
    `成本：长枪=${formatMoney(weaponCost)}，10%卷轴=${formatMoney(
...
if (typeof process !== "undefined" && /spear\.(ts|js)$/.test(process.argv[1] ?? "")) {
  run();
}
